...
- **Vanilla TypeScript** for motion and interaction (`src/scripts/`), about 7 KB gzipped on the home page. The terminal (press `` ` ``, Claude Code style slash commands) and the other easter eggs (`src/scripts/eggs.ts`: Shift five times for Spider-Man mode, type "boy" for Kratos and Ghost of Sparta mode) are split out and load on first use.
